...
export declare namespace PreModifiers {
    interface Options {
...
export class PreModifiers {
    constructor(private readonly options: PreModifiers.Options) {}
    ...
    public async get(
        guid: string,
        request: HotschedulesToast.config.PreModifierRequest
    ): Promise<HotschedulesToast.config.PreModifier> {
        ...
        if (_response.ok) {
            return await serializers.config.PreModifier.parseOrThrow(
                _response.body as serializers.config.PreModifier.Raw,
     ...
     */
    public async getAll(
        request: HotschedulesToast.config.PreModifiersRequest
    ): Promise<HotschedulesToast.config.PreModifier[]> {
        ...
        if (_response.ok) {
            return await serializers.config.preModifiers.getAll.Response.parseOrThrow(
                _response.body as serializers.config.preModifiers.getAll.Response.Raw,
                { allowUnknownKeys: true }
            );
        }
